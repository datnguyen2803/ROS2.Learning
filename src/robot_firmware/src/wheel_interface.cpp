...
namespace robot_firmware_interfaces
{
	...
	hardware_interface::return_type WheelInterface::write(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*duration*/)
	{
		...
		double delta_counts = left_motor_counts_per_loop - right_motor_counts_per_loop;

		// if((left_motor_counts_per_loop < 5.0) && (left_motor_counts_per_loop > -5.0))
		// {
		// 	left_motor_counts_per_loop *= 18.0;
		// }
		// else if((left_motor_counts_per_loop < 10.0) && (left_motor_counts_per_loop > -10.0))
		// {
		// 	left_motor_counts_per_loop *= 9.0;
		// }
		// else if((left_motor_counts_per_loop < 15.0) && (left_motor_counts_per_loop > -15.0))
		// {
		// 	left_motor_counts_per_loop *= 6.0;
		// }
		// else if((left_motor_counts_per_loop < 30.0) && (left_motor_counts_per_loop > -30.0))
		// {
		// 	left_motor_counts_per_loop *= 3.0;
		// }

		// if((right_motor_counts_per_loop < 5.0) && (right_motor_counts_per_loop > -5.0))
		// {
		// 	right_motor_counts_per_loop *= 18.0;
		// }
		// else if((right_motor_counts_per_loop < 10.0) && (right_motor_counts_per_loop > -10.0))
		// {
		// 	right_motor_counts_per_loop *= 9.0;
		// }
		// else if((right_motor_counts_per_loop < 15.0) && (right_motor_counts_per_loop > -15.0))
		// {
		// 	right_motor_counts_per_loop *= 6.0;
		// }
		// else if((right_motor_counts_per_loop < 30.0) && (right_motor_counts_per_loop > -30.0))
		// {
		// 	right_motor_counts_per_loop *= 3.0;
		// }

		if (left_motor_counts_per_loop < 0 && right_motor_counts_per_loop > 0)
		{
			if (left_motor_counts_per_loop > -THRESHOLD)
			{
				left_motor_counts_per_loop = -THRESHOLD;
			}
			if (right_motor_counts_per_loop < THRESHOLD)
			{
				right_motor_counts_per_loop = THRESHOLD;
			}
		}
		else if (left_motor_counts_per_loop > 0 && right_motor_counts_per_loop < 0)
		{
			if (left_motor_counts_per_loop < THRESHOLD)
			{
				left_motor_counts_per_loop = THRESHOLD;
		...
		{
			int sign = 1;
			if (left_motor_counts_per_loop < 0)
				sign = -1;
			auto min = std::ref(left_motor_counts_per_loop);
			auto max = std::ref(right_motor_counts_per_loop);
			if (std::abs(right_motor_counts_per_loop) < std::abs(left_motor_counts_per_loop))
			{
				min = std::ref(right_motor_counts_per_loop);
				max = std::ref(left_motor_counts_per_loop);
			}
			if (min != 0)
			{
				if (std::abs(min) < THRESHOLD)
				{
					double difference = std::abs(max) - std::abs(min);
					min.get() = sign * THRESHOLD;
					max.get() = sign * (THRESHOLD + difference);
				}
			}
			else if (max != 0)
			{
				if (std::abs(max) < THRESHOLD)
				{
					max.get() = sign * THRESHOLD;
				}
			}
		}

		if(((left_motor_counts_per_loop > 0) && (right_motor_counts_per_loop > 0))
		|| ((left_motor_counts_per_loop < 0) && (right_motor_counts_per_loop < 0)))
		{
			right_motor_counts_per_loop = right_motor_counts_per_loop * 0.987;
		}
